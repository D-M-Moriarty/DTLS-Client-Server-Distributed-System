...
public class DTLSOverDatagram {
    ...
    private static final String trustFilename = "/Users/darrenmoriarty/Development/TestDTLS/src/things/truststore.jks";
...
//    private static final String trustFilename = System.getProperty("test.src", ".") + "/" + pathToStores + "/" + trustStoreFile;
    ...
    private static final ByteBuffer serverApp = ByteBuffer.wrap("Hi Client, I'm Server".getBytes());
    private static final ByteBuffer clientApp = ByteBuffer.wrap("Hi Server, I'm Client".getBytes());
    ...
    static {
        ...
        try {
            DatagramSocket.setDatagramSocketImplFactory(new ServerCallable.CustomDatagramSocketImplFactory());
        } catch (IOException e) {
...
//        testCase.runTest(testCase);
        testCase.doClientSide(new DatagramSocket(), new InetSocketAddress(3000));
    }
   ...
   108      */
    void doServerSide(DatagramSocket socket, InetSocketAddress clientSocketAddr)
        ...
        ServerCallable.printHex("Server received application data", appData);
        ServerCallable.printHex("Server expected application data", clientApp);
    ...
    void doClientSide(DatagramSocket socket, InetSocketAddress serverSocketAddr)
        ...
        ServerCallable.printHex("Client received application data", appData);
        ServerCallable.printHex("Client expected application data", serverApp);
    ...
    ByteBuffer handshake(SSLEngine engine, DatagramSocket socket,
        ...
        while (!endLoops &&
            ...
            if (hs == SSLEngineResult.HandshakeStatus.NEED_UNWRAP ||
                ...
                ServerCallable.log(side, "Need DTLS records, handshake status is " + hs);
                ...
                if (hs == SSLEngineResult.HandshakeStatus.NEED_UNWRAP) {
                    ...
                    try {
                        ServerCallable.log(side, "handshake(): wait for a packet");
                        socket.receive(packet);
                        ServerCallable.log(side, "handshake(): received a packet, length = "
                                + packet.getLength());
                    } catch (SocketTimeoutException ste) {
                        ServerCallable.log(side, "Warning: " + ste);

                        packets.clear();
                        boolean finished = onReceiveTimeout(
                                engine, peerAddr, side, packets);

                        ServerCallable.log(side, "handshake(): receive timeout: re-send "
                        ...
                        if (finished) {
                            ServerCallable.log(side, "Handshake status is FINISHED "
                                    + "after calling onReceiveTimeout(), "
                                    + "finish the loop");
                            endLoops = true;
                        }

                        ServerCallable.log(side, "New handshake status is "
                ...
                if (rs == SSLEngineResult.Status.OK) {
                    ...
                    ServerCallable.log(side, "BUFFER_OVERFLOW, handshake status is " + hs);

                    // the client maximum fragment size config does not work?
                    throw new Exception("Buffer overflow: " +
                            "incorrect client maximum fragment size");
                } else if (rs == SSLEngineResult.Status.BUFFER_UNDERFLOW) {
                    ServerCallable.log(side, "BUFFER_UNDERFLOW, handshake status is " + hs);
                ...
                if (hs == SSLEngineResult.HandshakeStatus.FINISHED) {
                    ServerCallable.log(side, "Handshake status is FINISHED, finish the loop");
                    endLoops = true;
                }
                ...
                ServerCallable.log(side, "handshake(): need wrap: send " + packets.size()
                        + " packets");
                for (DatagramPacket p : packets) {
                    socket.send(p);
                }
                ...
                ServerCallable.log(side, "Handshake status is NOT_HANDSHAKING,"
                        + " finish the loop");
                endLoops = true;
        ...
        if (isServer) {
            appBuffer = receiveAppData(engine, socket, side, (Callable) () -> {
                ...
                SSLEngineResult.HandshakeStatus hs = engine.getHandshakeStatus();
                ServerCallable.log(side, "receiveAppData(): handshake status is " + hs);

                packets.clear();
                produceHandshakePackets(engine, peerAddr, side, packets);

                ServerCallable.log(side, "receiveAppData(): re-send " + packets.size()
                        + " final packets");
                for (DatagramPacket p : packets) {
                    socket.send(p);
                }
        ...
        SSLEngineResult.HandshakeStatus hs = engine.getHandshakeStatus();
        ServerCallable.log(side, "Handshake finished, status is " + hs);
        ...
        ServerCallable.log(side, "Negotiated protocol is " + session.getProtocol());
        ServerCallable.log(side, "Negotiated cipher suite is " + session.getCipherSuite());
    ...
    void sendAppData(SSLEngine engine, DatagramSocket socket,
        ...
        appData.flip();
        ServerCallable.log(side, "sendAppData(): send " + packets.size() + " packets");
        for (DatagramPacket p : packets) {
            socket.send(p);
        }
    }
    ...
    ByteBuffer receiveAppData(SSLEngine engine, DatagramSocket socket,
        ...
        while ((serverException == null) && (clientException == null)) {
            ...
            byte[] buf = new byte[BUFFER_SIZE];
            DatagramPacket packet = new DatagramPacket(buf, buf.length);
            ServerCallable.log(side, "receiveAppData(): wait for a packet");
            try {
                socket.receive(packet);
                ServerCallable.log(side, "receiveAppData(): received a packet");
            } catch (SocketTimeoutException e) {
                ServerCallable.log(side, "Warning: " + e);
                continue;
            }
            ...
            ServerCallable.log(side, "receiveAppData(): engine status is " + rs);
            recBuffer.flip();
    ...
    boolean produceHandshakePackets(SSLEngine engine, SocketAddress socketAddr,
        ...
        while (!endLoops &&
            ...
            if (rs == SSLEngineResult.Status.BUFFER_OVERFLOW) {
                ...
                ServerCallable.log(side, "produceHandshakePackets(): BUFFER_UNDERFLOW");
                ServerCallable.log(side, "produceHandshakePackets(): handshake status: " + hs);
                // bad packet, or the client maximum fragment size
                // config does not work?
                if (hs != SSLEngineResult.HandshakeStatus.NOT_HANDSHAKING) {
                    throw new Exception("Buffer underflow: " +
            ...
            if (hs == SSLEngineResult.HandshakeStatus.FINISHED) {
                ServerCallable.log(side, "produceHandshakePackets(): "
  ...
  /*
 607      * =============================================================
 608      * The remainder is support stuff to kickstart the testing.
 609      */
    ...
    public final void runTest(DTLSOverDatagram testCase) throws Exception {
        try (DatagramSocket serverSocket = new DatagramSocket();
            ...
            try {
                ...
                client = pool.submit(new ServerCallable.ClientCallable(
